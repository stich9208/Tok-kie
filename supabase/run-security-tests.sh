...
psql "$TOKKIE_SECURITY_TEST_DATABASE_URL" -X -v ON_ERROR_STOP=1 -f "$script_dir/security-test.sql"
exec psql "$TOKKIE_SECURITY_TEST_DATABASE_URL" -X -v ON_ERROR_STOP=1 -f "$script_dir/security-test.sql"
